...
import { qualityInstructionUploadService, qualityInstructionGetCurrentService, qualityInstructionGetCurrentFileService, qualityInstructionGetAllHistoryService, qualityInstructionGetHistoryFileService, qualityInstructionModifyService, qualityInstructionApproveService, qualityInstructionDeleteService } from 'services';

import QSInstructionAddView from './QSInstructionAddView';
import QSInstructionDetailView from './QSInstructionDetailView';
import QSInstructionModifyView from './QSInstructionModifyView';
import QSInstructionMDView from './QSInstructionMDView';

const FormItem = Form.Item;
const Option = Select.Option;

class QSInstructionComponent extends React.Component{
  ...
  getCurrent = () => {
    const addr = qualityInstructionGetCurrentService;
    $.ajax({
      type: "get",
      url: qualityInstructionGetCurrentService,
      async: false,
      success:function(d){
        this.setState({
	  item: d.data
  ...
  getAllApprove = () => {
    $.ajax({
      type: "get",
      url: "http://119.23.38.100:8080/cma/OperatingInstruction/getApprove",
      async: false,
      success:function(d){
  ...
  refreshData = () => {
    this.setState({
      data: getStore().getState().QSinstruction.items
    });
  }

  downloadFile = (props) => {
    let curId = props.item.id;
    const url = qualityInstructionGetHistoryFileService + "?id=" + curId;
    var tempwindow = window.open();
    tempwindow.location = url;
  }

  showDetail = (props) => {
    this.props.addTab('历史详情','历史详情',QSInstructionDetailView,props);
  }

  inspectUnapprove = (unProps) => {
    this.props.addTab('修改详情','修改详情',QSInstructionMDView,unProps);
  }
  ...
  deleteHistory = (props) => {
    ...
    $.ajax({
      type: "post",
      url: qualityInstructionDeleteService,
      data: {
        id: curId,
      },
      async: false,
  ...
  downloadCurrent = () => {
    const url = qualityInstructionGetCurrentFileService;
    var tempwindow = window.open();
    tempwindow.location = url;
  }
  ...
  addNewManual = () => {
    ...
    this.props.addTab('添加作业指导书','添加作业指导书',QSInstructionAddView,props);
  }
  ...
  modifyManual = () => {
    ...
    this.props.addTab('修改作业指导书','修改作业指导书',QSInstructionModifyView,props);
  }
  ...
  handleApprove = (value,unProps) => {
    ...
    $.ajax({
      type: "post",
      url: qualityInstructionApproveService,
      data: {
        id: curId,
	state: state,
      },
...
export default QSInstructionComponent;
